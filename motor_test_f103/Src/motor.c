...
static MotorState last_drive_state[2] = {MOTOR_COAST, MOTOR_COAST};
...
static const MotorChInfo ch_info[2] = {
    {&htim3, TIM_CHANNEL_2, MOTOR1_ARR},
...
static void Motor_TIM3_PWM_Init(void)
{
    ...
    gpio.Pin = MOTOR1_PWM_PIN | MOTOR2_PWM_PIN;
...
{
    /* Enable AFIO clock for SWJ remap (PB3 as GPIO) */
    ...
    /* Pull all INA/INB high (coast) for safe power-up */
...
void Motor_SetState(MotorChannel ch, MotorState state)
{
    ...
    case MOTOR_FWD:
        ...
        last_drive_state[ch] = MOTOR_FWD;
        break;
    case MOTOR_REV:
        HAL_GPIO_WritePin(p->ina_port, p->ina_pin, GPIO_PIN_SET);
        HAL_GPIO_WritePin(p->inb_port, p->inb_pin, GPIO_PIN_RESET);
        last_drive_state[ch] = MOTOR_REV;
        break;
    case MOTOR_COAST:
        HAL_GPIO_WritePin(p->ina_port, p->ina_pin, GPIO_PIN_SET);
        HAL_GPIO_WritePin(p->inb_port, p->inb_pin, GPIO_PIN_SET);
...
{
    if (last_drive_state[ch] == MOTOR_REV) {
...
{
    if (last_drive_state[ch] == MOTOR_FWD) {
        Motor_SetState(ch, MOTOR_BRAKE);
        HAL_Delay(MOTOR_DIR_DEADTIME_MS);
    }
